...
baseinstall: ssh install base init init-sub keymap keyd grub autostart cron keyring tlp emacs-mozc icons gist fonts emacs-restore
...
# HOMEキーで最小化Emasを最大化する
emacs-restore: ## emacs-restore スクリプトのシンボリックリンク作成
	sudo ln -vsfn ${PWD}/bin/emacs-restore /usr/local/bin/emacs-restore
	sudo chmod +x /usr/local/bin/emacs-restore
...
##   Step 2: make mattermost             # コンテナ起動（docker/Makefile）
##   Step 3: make mattermost-restore-db  # DBリストア
##
########################################################
.PHONY: docker-install polkit docker-setup mattermost-restore-db
...
docker-setup: polkit ## 【Step1】データディレクトリ作成＋Dropboxからrsyncで復元
	mkdir -p /home/minoru/Docker/mattermost/{db,data,config,logs,plugins}
	sudo chown -R 2000:2000 /home/minoru/Docker/mattermost
	for dir in data config logs plugins; do \
		rsync -av /home/minoru/Dropbox/backup/mattermost/$$dir/ /home/minoru/Docker/mattermost/$$dir/; \
	done
	mkdir -p /home/minoru/Docker/gitea/data
	rsync -av /home/minoru/Dropbox/backup/gitea/ /home/minoru/Docker/gitea/data/
	@echo "✓ 完了。次: make mattermost（docker/Makefile）"

mattermost-restore-db: ## 【Step3】DBをDropboxのmattermost.sqlからリストア
	docker stop mattermost
	docker exec -i mattermost-postgres psql -U mattermost -d postgres -c "DROP DATABASE mattermost;"
	docker exec -i mattermost-postgres psql -U mattermost -d postgres -c "CREATE DATABASE mattermost;"
	grep -v '\\restrict' /home/minoru/Dropbox/backup/mattermost/mattermost.sql \
		| docker exec -i mattermost-postgres psql -U mattermost -d mattermost
