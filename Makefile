...
baseinstall: ssh install base init keymap grub autostart myjob autobackup keyring tlp emacs-mozc icons gist fonts
...
automerge: ## automerge.sh のシンボリックリンク作成（cron自動実行用）
	sudo ln -vsfn ${PWD}/crontab/automerge.sh /usr/local/bin
	sudo chmod +x /usr/local/bin/automerge.sh

autobackup: ## autobackup.sh のシンボリックリンク作成（cron自動実行用）
	sudo ln -vsfn ${PWD}/crontab/autobackup.sh /usr/local/bin
