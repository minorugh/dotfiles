...
baseinstall: ssh install base init keymap grub autostart cron keyring tlp emacs-mozc icons gist fonts
...
autobackup: ## autobackup.sh / mattermost-backup.sh のシンボリックリンク作成（cron自動実行用、P1のみ）
	...
	sudo chmod +x /usr/local/bin/moz-backup.sh
...
# Gmail はOAuth2認証なので2段階認証設定必須
	...
	cp -a ${HOME}/Dropbox/backup/thunderbird/.thunderbird ${HOME}/.thunderbird
