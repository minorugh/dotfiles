...
baseinstall: ssh install base init init-sub keymap keyd grub autostart cron emacs-trash keyring tlp emacs-mozc icons gist fonts emacs-toggle
...
autobackup: ## cron スクリプト群のシンボリックリンク作成（cron自動実行用、P1のみ）
	...
	sudo ln -vsfn ${PWD}/backup/mattermost-backup.sh /usr/local/bin
...
# 別館が存在し、かつ中にMakefileがある時だけ静かに実行
	if [ -f $(ENV_SOURCE_DIR)/git/Makefile ]; then \
