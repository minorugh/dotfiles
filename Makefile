...
dropbox-restart: ## サスペンド復帰時にDropbox同期を自動再起動（PrepareForSleep監視、systemd --user）
	chmod +x ${PWD}/bin/dropbox-restart.sh
	mkdir -p ${HOME}/.config/systemd/user
	ln -vsf ${PWD}/etc/systemd/user/dropbox-restart.service \
		${HOME}/.config/systemd/user/dropbox-restart.service
	systemctl --user daemon-reload
	systemctl --user enable --now dropbox-restart.service
	systemctl --user status dropbox-restart.service --no-pager
