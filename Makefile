...
# PATH は .zshrc の GOROOT/GOPATH 設定で管理

ghq: ## ghq のインストール（リポジトリ管理ツール）
	go install github.com/x-motemen/ghq@latest
# go 1.23 以上が必要
# GOPATH は ~/src/go（.zshrc で設定済み）
...
lepton: ## Lepton (Gist クライアント) AppImage のインストール
	mkdir -p ${HOME}/Apps
	cd ${HOME}/Apps && \
	wget https://github.com/hackjutsu/Lepton/releases/download/v1.10.0/Lepton-1.10.0.AppImage
	chmod +x ${HOME}/Apps/Lepton-1.10.0.AppImage
# バージョンアップ時は URL とファイル名の数字を変更すること
# alias は .zshrc で設定済み（~/Apps/Lepton-1.10.0.AppImage）
...
########################################################
## Docker 環境セットアップ（新規マシン用）
########################################################
docker-install: ## Docker Engine + Compose のインストール（公式リポジトリ経由）
	sudo apt update
...
polkit: ## polkit認証ダイアログ抑制（Docker用、リストア時に実行）
	printf '%s\n' \
...
docker-setup: polkit ## Docker環境の初期セットアップ（インストール後に実行）
	mkdir -p ${HOME}/Dropbox/docker-data/mattermost/{config,data,logs,plugins,client/plugins,bleve-indexes}
	sudo chown -R 2000:2000 ${HOME}/Dropbox/docker-data/mattermost
	@echo "✓ Docker データディレクトリ作成完了"

mattermost: ## Mattermost の起動（pull + chown + up）
	docker compose -f ${PWD}/docker/mattermost/docker-compose.yml pull
	mkdir -p ${HOME}/docker-data/mattermost/{config,data,logs,plugins}
	sudo chown -R 2000:2000 ${HOME}/docker-data/mattermost
	docker compose -f ${PWD}/docker/mattermost/docker-compose.yml up -d
	@echo "✓ Mattermost 起動完了"
