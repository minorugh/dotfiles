...
スクリプト本体: `~/src/github.com/minorugh/dotfiles/crontab/automerge.sh` → `/usr/local/bin/automerge.sh`
ログ: `/tmp/automerge.log`
...
### 3. autobackup.sh（23:50）

myjob.sh 完了後に続けてDropboxバックアップ一式を実行。
ログ: `/tmp/autobackup.log`（myjob.shと同じファイルに追記）
