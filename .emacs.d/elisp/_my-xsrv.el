...
;;; 通常の `dired' には一切影響しない。
;;; 70-hydra-dired.el から (require 'my-xsrv) で読み込む想定。
;;; Code:
...
;; 現在の dired バッファの先頭2行（パス行・合計行）に my-xsrv-2pane-header-face を上書きする.
;; `face' ではなく `font-lock-face' を使うことで、dired の
;; font-lock 再描画によって上書き消去されないようにしている。"
(defun my-xsrv-2pane--fontify-header ()
  "Overwrite the first two lines of the buffer with `font-lock-face'."
  (save-excursion
    (goto-char (point-min))
