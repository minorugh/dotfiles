...
(defvar gcal-widget-org-file
  (locate-user-emacs-file "tmp/gcal.org")
  "Auto-generated org file synced from Google Calendar.
Do not edit by hand.")

(unless (file-exists-p gcal-widget-org-file)
  (make-empty-file gcal-widget-org-file t))

(defcustom gcal-widget-months-back 12
...
(defcustom gcal-widget-block-end-date-correction 1
  "Number of days to add to diary-block end dates."
  :type 'integer :group 'gcal-widget)

(defun gcal-widget--fix-block-end-dates (input-file output-file days)
...
(defun gcal-widget--read-url (file)
...
(defun gcal-widget--kill-file-buffer (file)
...
(defun gcal-widget--group-diary-lines (lines)
...
(defun gcal-widget--filter-recent (input-file output-file months-back)
  ...
  (let* ((cutoff-abs (- (calendar-absolute-from-gregorian
         ...
         (groups (gcal-widget--group-diary-lines lines)))
    (with-temp-buffer
      (dolist (g groups)
        (let* ((head (car g))
               (date-abs
...
(defun gcal-widget--parse-sexp-line (line)
...
(defun gcal-widget--block-dates (sexp)
  "Return block dates from SEXP, or nil."
  (let ((inner (and (eq (car-safe sexp) 'and) (cadr sexp))))
    (when (eq (car-safe inner) 'diary-block)
      (cdr inner))))

(defun gcal-widget--org-timestamp (date)
  "Format DATE as an Org timestamp."
  (format-time-string "%Y-%m-%d %a"
                       (encode-time 0 0 0 (nth 1 date) (nth 0 date) (nth 2 date))))

(defun gcal-widget--format-org-entry (text date)
  "Format TEXT and DATE as an Org entry."
  (format "* %s\n  <%s>\n" text (gcal-widget--org-timestamp date)))

(defun gcal-widget--format-org-block-entries (text start end)
  ...
  (let ((day  (calendar-absolute-from-gregorian start))
        ...
        (out ""))
    (while (<= day last)
      (setq out (concat out (gcal-widget--format-org-entry
                              text (calendar-gregorian-from-absolute day))))
      (setq day (1+ day)))
    out))

(defun gcal-widget--format-org-sexp-entry (text sexp)
  "Format TEXT and SEXP as an Org diary entry."
  (format "* %s\n  <%%%%%S>\n" text sexp))

(defun gcal-widget--diary-group-to-org (group)
  "Convert GROUP from diary format to Org format."
  (let ((head (car group)))
    (cond
     ;; 単日: M/D/YYYY TEXT
     ((string-match "\\`\\([0-9]+\\)/\\([0-9]+\\)/\\([0-9]+\\) *\\(.*\\)\\'" head)
      (gcal-widget--format-org-entry
       (match-string 4 head)
       (list (string-to-number (match-string 1 head))
             (string-to-number (match-string 2 head))
             (string-to-number (match-string 3 head)))))
     ;; sexp形式(diary-block / diary-cyclic 等)
     (t (let ((parsed (gcal-widget--parse-sexp-line head)))
          (when parsed
            (let* ((sexp  (car parsed))
                   (text  (cdr parsed))
                   (block (gcal-widget--block-dates sexp)))
              (if block
                  (gcal-widget--format-org-block-entries
                   text
                   (list (nth 0 block) (nth 1 block) (nth 2 block))
                   (list (nth 3 block) (nth 4 block) (nth 5 block)))
                (gcal-widget--format-org-sexp-entry text sexp)))))))))

(defun gcal-widget--diary-to-org (input-file output-file)
  ...
  (let* ((lines  (with-temp-buffer
    ...
    (with-temp-buffer
      (dolist (g groups)
        (let ((org-entry (gcal-widget--diary-group-to-org g)))
          (if org-entry
              (insert org-entry)
            (message "gcal-widget-sync: 未対応形式のためスキップ: %s" (car g)))))
      (write-region (point-min) (point-max) output-file))))

;; URLファイルが見つからないカレンダーはエラーにせずスキップする。
(defun gcal-widget-sync ()
  "Sync all calendars in `gcal-widget-calendars' into `gcal-widget-org-file'.
Builds the merged result in a temp file first, and only replaces
`gcal-widget-org-file' once every calendar has been processed
...
org file in a half-written state."
  ...
  (let ((count 0)
    ...
    (unwind-protect
        (progn
          (dolist (cal gcal-widget-calendars)
            (let* ((name (car cal))
                   (url (gcal-widget--read-url (cdr cal))))
              (if (not url)
                  (message "gcal-widget-sync: %s のURLが見つかりません(%s), skip"
                           name (cdr cal))
                (let* ((tmp-ics      (make-temp-file "gcal-sync-" nil ".ics"))
                  ...
                  (unwind-protect
                      (progn
                        ...
                        ;; 3. 複数日イベントの終了日を補正
                        (gcal-widget--fix-block-end-dates
                         tmp-raw tmp-fixed gcal-widget-block-end-date-correction)
                        ;; 4. 日付でフィルタ
                        (gcal-widget--filter-recent tmp-fixed tmp-filtered gcal-widget-months-back)
                        ;; 5. org形式に変換
                        (gcal-widget--diary-to-org tmp-filtered tmp-org-part)
                    ...
                    (dolist (f (list tmp-ics tmp-raw tmp-fixed tmp-filtered tmp-org-part))
                      (gcal-widget--kill-file-buffer f)
                      (when (file-exists-p f) (delete-file f))))))))
          ;; 1件以上成功していれば、まとめて本番ファイルへ反映する
          (when (> count 0)
            (copy-file tmp-org gcal-widget-org-file t)))
      (gcal-widget--kill-file-buffer tmp-org)
  ...
  (when-let* ((time (or (org-get-scheduled-time (point))
    ...
    (let ((dow (string-to-number (format-time-string "%w" time))))
      (concat (format-time-string dashboard-agenda-time-string-format time)
              (format "（%s）" (aref gcal-widget-weekday-kanji dow))))))
...
(defun gcal-widget--sync-on-exit ()
  "Sync Google Calendar on exit, ignoring errors and timeouts."
  (with-timeout (10 (message "gcal-widget-sync: タイムアウトのためスキップ"))
    (ignore-errors (gcal-widget-sync))))

(add-hook 'kill-emacs-hook #'gcal-widget--sync-on-exit)
