...
;;;
;;; カテゴリ中心の設計:
;;;   my-howm-categories / my-howm-special-entries (2章) が全ての起点。
;;;   フェイス・font-lock・作成メニュー・検索は全てこの2つのリストを
;;;   参照して動く。カテゴリを増減・変更するときはまずここを見ること。
;;;
...
;;;   4. Font-lock  (カテゴリ色付け)
;;;   5. 新規メモ作成
;;;   6. 特殊エントリ実装: Junk (Perlスクラッチ)
;;;   7. カテゴリ検索
;;;   8. ゴミ箱へ移動
;;;   9. キーバインド
;;;
;;; NOTE: howm-mode-hook (font-lock 適用) は (leaf howm ...) の :preface に
;;; 配置している。:config 内で add-hook すると、バイトコンパイル時点で
;;; howm がまだロードされておらず font-lock-add-keywords が効かないため。
;;; 9章のキーバインドより後、ファイル末尾に置いてあるので探すときは注意。
;;; Code:
...
通常カテゴリは新規 howm ファイルを作成。特殊エントリは外部関数を呼び出す。"
  ...
  ;; 6. 特殊エントリ実装: Junk (Perlスクラッチ)
  ;;
  ;;  my-howm-special-entries (2章) の ?c エントリから呼ばれる。
  ;;  他の特殊エントリ(my-haiku-note-post/my-haiku-note)は外部の
  ;;  70-*.el 側で定義されているが、これは単純なので howm 側に置く。
  ;; ============================================================
