...
(leaf flycheck
  ...
  :bind (("C-c r" . my/textlint-check-region)  ; 基本機能: リージョン(なければ全体)チェック
	 ("C-c d" . my/diary-textlint-today))   ; diary.txt専用: 今日の分を自動選択してチェック
  :config
  ...
  ;; (defun my-toggle-textlint ()
  ;;   "現在のバッファでtextlintチェッカーのon/offを切り替える。"
  ;;   (interactive)
  ;;   (if (memq 'textlint flycheck-disabled-checkers)
  ;;       (progn
  ;;         (setq-local flycheck-disabled-checkers
  ;;                     (delq 'textlint flycheck-disabled-checkers))
  ;;         (message "textlint: ON"))
  ;;     (progn
  ;;       (setq-local flycheck-disabled-checkers
  ;;                   (cons 'textlint flycheck-disabled-checkers))
  ;;       (message "textlint: OFF")))
  ;;   (flycheck-buffer)))

  :init
  (defvar-local my/textlint-overlays nil
    "textlintの指摘を表示しているoverlayのリスト。")

  (defun my/textlint-clear-overlays ()
    (mapc #'delete-overlay my/textlint-overlays)
    (setq my/textlint-overlays nil))

  (defun my/textlint-check-region (start end)
    "リージョン（非選択時はバッファ全体）をtextlintでチェックし、
該当箇所にその場で波線overlayを表示する。基本機能はこれだけ。"
    (interactive (if (use-region-p)
                     (list (region-beginning) (region-end))
                   (list (point-min) (point-max))))
    (my/textlint-clear-overlays)
    (let* ((text (buffer-substring-no-properties start end))
           (base-line (line-number-at-pos start))
		...
		(insert text)
		(with-temp-buffer
                  (call-process-region text nil "textlint" nil t nil
                                       "--no-color" "--stdin" "--stdin-filename" "region.txt")
                  (buffer-string))))))
	...
	(let ((count 0))
          (dolist (line (split-string result "\n"))
            (when (string-match
              ...
              (let* ((rel-line (string-to-number (match-string 1 line)))
                     (col (string-to-number (match-string 2 line)))
                     (msg (match-string 4 line))
                     (real-line (+ base-line rel-line -1)))
		(save-excursion
                  (goto-char (point-min))
                  (forward-line (1- real-line))
                  (forward-char col)
                  (let* ((ov-end (min (line-end-position) (+ (point) 8)))
			 (ov (make-overlay (point) ov-end)))
                    (overlay-put ov 'face '(:underline (:color "red" :style wave)))
                    (overlay-put ov 'help-echo msg)
                    (push ov my/textlint-overlays)))
		(setq count (1+ count)))))
          (message (if (zerop count)
                       "textlint: 指摘なし ✓"
                     (format "textlint: %d件の指摘あり（該当箇所にカーソルを合わせると内容表示）" count)))))))

  (defun my/diary-textlint-today ()
    "diary.txt専用: 今日のセクションを自動判定し、選択せず my/textlint-check-region を呼ぶ。"
    (interactive)
    (unless (and (buffer-file-name)
		 (string-match-p "diary\\.txt\\'" (buffer-file-name)))
      (user-error "diary.txt 用のコマンドです"))
    (save-excursion
      (goto-char (point-min))
      (unless (re-search-forward "^\\*\\[" nil t)
	(user-error "日付ヘッダー（*[YYYYMMDD]）が見つかりません"))
	...
	(let ((end (if (re-search-forward "^\\*\\[" nil t)
                       (match-beginning 0)
                     (point-max))))
          (my/textlint-check-region start end)))))
  )
