...
- automerge.sh 完了後に `dotfiles/cron/backup/Makefile` の各ターゲットを個別に呼び出してバックアップ一式を実行
- ログ: `/tmp/cron.log`
...
詳細は `dotfiles/cron/backup/Makefile` のコメント参照。
...
### dotfiles/cron/backup/Makefile（夜間自動バックアップ）

`autobackup.sh` から毎夜 23:50 に呼び出される。
手動実行: `make -f ~/src/github.com/minorugh/dotfiles/cron/backup/Makefile <ターゲット名>`
