...
- xsrv-backup.sh / xsrv-backup-data.sh は systemd-user timer で管理（cron では SSH エージェントを引き継げないため）
...
# emacs-trash-sweep.sh: Emacs ゴミ箱をシステムゴミ箱へ移送（毎月1日 23:45）
45 23 1 * * /usr/local/bin/emacs-trash-sweep.sh >> /tmp/cron.log 2>&1
```

注: xmodmap のジョブは `~/.xprofile` に移行済み
注: xsrv-backup.sh / xsrv-backup-data.sh は systemd-user timer で管理（後述）
...
### 3.3 emacs-trash-sweep.sh (毎月1日 23:45)
...
### 3.4 xsrv-backup.sh（systemd-user timer: 毎日 22:05）
...
| ドメイン | xserver 本番 | ローカル | Gitea |
|---|---|---|---|
| gospel-haiku.com | `/home/minorugh/gospel-haiku.com/public_html/` | `~/src/github.com/minorugh/xsrv-GH/` | `localhost:3000/minoru/xsrv-GH` |
| minorugh.com | `/home/minorugh/minorugh.com/public_html/` | `~/src/github.com/minorugh/xsrv-minorugh/` | `localhost:3000/minoru/xsrv-minorugh` |
...
make             # 緊急停止（xsrv-backup / xsrv-backup-data 両方停止）
make xsrv-start  # 再開
...
Emacs の hydra から `make -k` 一発で両バックアップ緊急停止できるよう設定済み。
...
`commit_lean()` 関数により、200件超えで履歴を自動リセット（`--orphan` + force push）。
手動の `make xsrv-commit-lean` は不要。

---

### 3.5 xsrv-backup-data.sh（systemd-user timer: 毎時 00分）

- gospel-haiku.com の4句会 data フォルダのみを対象に1時間毎に同期
- リアルタイムで変化する投句・選句データの頻繁なバックアップが目的
- 変更なしの場合は commit をスキップ
- push先は `xsrv-GH`（xsrv-backup.sh と同じリポジトリ）
- ログ: `/tmp/xsrv-backup-data.log`

#### バックアップ対象

| 句会 | パス |
|---|---|
| 毎日句会 | `gospel-haiku.com/public_html/d_kukai/data/` |
| 月例句会 | `gospel-haiku.com/public_html/m_kukai/data/` |
| 吟行句会 | `gospel-haiku.com/public_html/s_kukai/data/` |
| 若鮎句会 | `gospel-haiku.com/public_html/w_kukai/data/` |

#### 衝突回避

`xsrv-backup-data.sh`（毎時00分）と `xsrv-backup.sh`（22:05）が同じリポジトリを使うため、
22時台の衝突を timer を5分ずらすことで回避。

#### commit履歴自動管理

`commit_lean()` 関数により、200件超えで履歴を自動リセット。
...
```
xsrv-backup-data.service
xsrv-backup-data.timer
```
...
```makefile
all: xsrv-stop xsrv-data-stop  # make -k で両バックアップ緊急停止
...
xsrv-data-stop:          # xsrv-backup-data 緊急停止（句会dataのため最重要）
xsrv-data-start:         # xsrv-backup-data 再開
xsrv-data-log:           # xsrv-backup-data ログ表示
xsrv-data-run:           # xsrv-backup-data 今すぐ手動実行

bat-status:              # バッテリー状態・しきい値を確認
bat-set-60:              # 充電上限を60%に変更（長期AC接続時推奨）
bat-set-80:              # 充電上限を80%に変更（外出前・標準運用）
bat-discharge:           # 強制放電開始（ACつなぎのまま放電・60%以下になったら停止）
...
cron/
  README.md              # このファイル
  Makefile               # 緊急操作パネル（make -k で xsrv 両バックアップ緊急停止）
  crontab                # P1 に適用する crontab 本体（Git 管理）
  automerge.sh           # 句会パスワード同期・マージ
  emacs-trash-sweep.sh   # Emacs ゴミ箱をシステムゴミ箱へ移送（月1回）
  xsrv-backup.sh         # xserver → ローカルへ rsync + git（全体、毎日22:05）
  xsrv-backup-data.sh    # xserver → ローカルへ rsync + git（4句会dataのみ、毎時）
```

注: バックアップ一族（autobackup.sh 等）は `dotfiles/backup/` に移動済み。詳細は `dotfiles/backup/README.md` 参照。
...
- **緊急停止は `dotfiles/cron/` で `make` または `make -k` を実行（両バックアップ同時停止）**
- commit履歴は200件超えで自動リセット（手動操作不要）
...
- リストア後に `xsrv-backup-data` の timer/service も手動で有効化すること：
